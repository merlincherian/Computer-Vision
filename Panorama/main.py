...
def lucas_kanade(H, I):
    ...
    Sx = np.array([[-1, 0, 1],
                        ...
                        [-1, 0, 1]]) / 8
    Sy = Sx.T
    I_x = convolve_img(I, -Sx)
    I_y = convolve_img(I, -Sy)
    I_t = I - H
...
def iterative_lucas_kanade(H, I, steps):
    ...
    for i in range(steps):
        ...
        translated = translate(H, disp)
        disp2, AtA, Atb = lucas_kanade(translated, I)
        disp = disp + disp2
...
def gaussian_pyramid(image, levels):
    ...
    for level in range(1, levels):
        ...
        convolved_img = convolve_img(image, gk)
        ...
        sampled_img = convolved_img[1::2, 1::2] #TODO Fix this
        # add the sampled image to the list
        pyr.append(sampled_img)
        image = sampled_img
    return pyr
...
def pyramid_lucas_kanade(H, I, initial_d, levels, steps):
    ...
    H_pyr = gaussian_pyramid(H, levels)[::-1]
    I_pyr = gaussian_pyramid(I, levels)[::-1]
...
def mosaic(images, initial_displacements, load_displacements_from):
    ...
    pano_width = images[0].shape[1]
    pano_height = images[0].shape[0]
    sum_u = 0
    sum_v = 0
    for i in final_displacements:
        u, v = abs(i[0]), abs(i[1])
        sum_u += u
        sum_v += v
    ...
    panorama = build_panorama(images, (int(np.ceil(pano_height)), int(np.ceil(pano_width))), final_displacements, initial_pos.copy())
